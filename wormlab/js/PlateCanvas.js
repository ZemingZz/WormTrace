 ...
 */
import { fmtElapsed, fmtHours } from './LifeCycle.js?v=9';
...
export class PlateCanvas {
  ...
  _openZoomModal() {
    ...
    modal.style.cssText =
      'position:fixed;inset:0;background:rgba(0,0,0,0.93);z-index:1200;' +
  ...
  _render({ plate, hrsElapsed = 0, stage, foodPct = 100, wormCount = 1, totalEggs = 0, inoculatedAt, population }) {
    ...
    if (plate.inoculatedAt) {
      ...
      const isDpy = String(plate.strainId ?? 'N2').startsWith('dpy-');
      const isMale = String(plate.strainId ?? '').includes('male');   // males drawn striped

      // Build the worm list (mixed-generation round-robin, or a single stage).
      // Each population group may carry its own `male`/`dpy` flags so a single plate
      // can show MULTIPLE worm types (e.g. a striped male strain + a dumpy strain).
      let items = [];
      if (population && population.length) {
        ...
        while (remaining > 0 && items.length < 200) {
          for (const g of groups) { if (g.left > 0) { items.push({ stageId: g.stageId, color: g.color, male: g.male, dpy: g.dpy }); g.left--; remaining--; } }
        }
      ...
      // ── Living worms ──
      if (items.length) this._layoutWorms(ctx, cx, cy, wormR, items, isDpy, isMale);
    }
  ...
  _drawOneWorm(ctx, wLen, wWid, color, swimPhase, isMale = false) {
    const N = 14;
    // Males are slimmer, hermaphrodites plumper — a real, visible C. elegans difference.
    const bodyW = Math.max(0.9, wWid * (isMale ? 1.7 : 2.3));
    const amp = wWid * 1.5 + wLen * 0.06;                   // gentle, worm-like undulation
    const pts = [];
    for (let s = 0; s <= N; s++) {
      const u = s / N;
      const bx = (u - 0.5) * wLen * 2;
      const taper = Math.sin(u * Math.PI);                 // thin at both ends
      const by = Math.sin(u * Math.PI * 1.8 - swimPhase) * amp * taper;
      pts.push([bx, by]);
    }
    ...
    trace(); ctx.strokeStyle = color; ctx.lineWidth = bodyW; ctx.stroke();
    trace(); ctx.strokeStyle = 'rgba(255,255,255,0.18)'; ctx.lineWidth = Math.max(0.35, bodyW * 0.35); ctx.stroke();
    // Head: hermaphrodites get a smooth rounded head in the body colour; MALES get a
    // distinct black head dot (with a faint ring) so the two are easy to tell apart.
    const head = pts[N];
    if (isMale) {
      ctx.beginPath(); ctx.arc(head[0], head[1], Math.max(1.0, bodyW * 0.7), 0, Math.PI * 2);
      ctx.fillStyle = '#0a0a0a'; ctx.fill();
      ctx.lineWidth = Math.max(0.35, bodyW * 0.22); ctx.strokeStyle = 'rgba(255,255,255,0.8)'; ctx.stroke();
    } else {
      ctx.beginPath(); ctx.arc(head[0], head[1], Math.max(0.7, bodyW * 0.55), 0, Math.PI * 2);
  ...
  _layoutWorms(ctx, cx, cy, maxR, items, isDpy, isMale = false) {
    const t = this._t;
    ...
    if (A.length < drawCount) {
      for (let i = A.length; i < drawCount; i++) {
        ...
        A.push({ nx: Math.cos(ang) * rr, ny: Math.sin(ang) * rr, dir: Math.random() * Math.PI * 2,
                 turn: 0, spd: 0.00030 + Math.random() * 0.00040, ph: Math.random() * Math.PI * 2 });
      }
    } else if (A.length > drawCount) {
      A.length = drawCount;
    }

    // Size scales with the canvas AND inversely with crowding: a near-empty plate
    // shows big, clearly-wiggling worms; a packed plate shows many small ones.
    const density = Math.min(1, drawCount / 120);
    const div = 2.0 + density * 4.0;                          // 2.0 (few) → 6.0 (crowded)
    const sizeScale = (this.canvas.width / 300) / div;
    ...
    for (let i = 0; i < drawCount; i++) {
      ...
      const itemDpy  = it.dpy  ?? isDpy;        // per-worm strain flags (mixed plates)
      const itemMale = it.male ?? isMale;
      let [wLen, wWid] = this._wormSize(it.stageId, itemDpy);
      wWid *= 1.9;                          // base widths are hairline-thin → give real body width
      wLen *= sizeScale; wWid *= sizeScale;
      const wx = cx + ag.nx * maxR, wy = cy + ag.ny * maxR;
      const swimPhase = isDauer ? ag.ph : (t * 0.06 + ag.ph);   // dauer: frozen pose, no undulation
      ctx.save();
      ctx.translate(wx, wy);
      ctx.rotate(ag.dir);                 // head points the way it's crawling
      this._drawOneWorm(ctx, wLen, wWid, it.color, swimPhase, itemMale);
      ctx.restore();
    }
    if (items.length > 160) {
  ...
  _drawCorpses(ctx, cx, cy, maxR) {
    ...
    const sizeScale = (this.canvas.width / 300) / 4.5;
