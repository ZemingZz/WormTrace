...
const PROCEDURES = [
  ...
  {
    id: 'chemotax', icon: '🧭', title: 'Chemotaxis Assay', tag: 'Odor preference / sensing',
    ...
    materials: [
      ...
      { amt: 1, unit: 'mL', name: '1 M MgSO₄ (→ 1 mM)' },
    ],
    steps: [
      'Make the agar: combine agar + water, autoclave, cool to ~55 °C, then swirl in the sterile KPO₄ / CaCl₂ / MgSO₄. Pour ~10 mL per 100 mm plate. Do NOT add bacteria — chemotaxis plates are foodless. Dry 1–2 days.',
      'Mark the plate bottom: two spots opposite each other ~0.5–1 cm from the edge — one “test” (odor), one “control” (solvent only) — plus a center “origin”.',
      'Make 1 M sodium azide (anesthetic): dissolve 0.65 g NaN₃ in 10 mL deionized water (keeps ~weeks at 4 °C). It freezes worms in place where they arrive. ⚠ NaN₃ is toxic — gloves, no skin/inhalation, dispose properly.',
      'Make the odor solution in its SOLVENT. Volatile odors are diluted in 100% ethanol — e.g. diacetyl or isoamyl alcohol 1:1000 in ethanol, benzaldehyde 1:200 in ethanol. (Just mix, e.g. 1 µL odor + 999 µL ethanol = 1:1000.)',
      'Spot 1 µL of 1 M sodium azide on BOTH the test and control spots.',
      'Spot 1 µL of the odor-in-ethanol on the TEST spot, and 1 µL of plain ethanol (no odor) on the CONTROL spot. Let the spots soak in ~few min.',
      'Wash 50–200 well-fed young adults off a plate with M9 buffer (or ddH₂O), let them settle, pipette them onto the center origin, and wick away the liquid with a tissue corner so they crawl freely.',
      'Lid on; leave undisturbed at room temp for 30–60 min.',
      'Count worms at the test spot (A), the control spot (C), and total scored (N). Chemotaxis Index = (A − C) / N. +1 = strong attraction, −1 = strong repulsion, ~0 = neutral/no preference.',
    ],
    notes: 'SOLVENT cheat-sheet — VOLATILE odors (diacetyl, isoamyl alcohol, benzaldehyde, pyrazine, 2-butanone) → dilute in 100% ethanol; the control spot is plain ethanol. WATER-SOLUBLE attractants (NaCl, cAMP, biotin, lysine) → dissolve in water or M9; the control spot is that same water/M9. Always run the solvent-only control on the SAME plate so the index subtracts any solvent effect. The azide and odor dilutions are made fresh in microliter amounts — they are NOT scaled by the batch slider (which only sizes the agar).',
  },
  {
